		...
		var selectNode = self.selectNode = function(node, el){
			...
			d3.select(el)
				...
				.transition()
					.duration(1000)
					.ease(d3.ease('elastic'))
					.attr('r', function(d){ return d.rad * selectedRadius})
		...
		*/
		...
		function zoomNone(d, i, el){
			...
			// var transition = d3.select(el)
				...
				.duration(d3.event.altKey ? 7500 : 700)
			...
			transition.filter("circle")
			    .attr("r", function(d) { return (d.zr = d.selected ? d.rad * selectedRadius : d.rad); });
		...
		function zoomNode(d, i, el) {
		    ...
		    xScale.domain([d.x - d.rad, d.x + d.rad]);
		    yScale.domain([d.y - d.rad, d.y + d.rad]);
		...
		*/
		...
		function pathForLink(d){
			...
			if (d.target && d.target.entity){
				...
				var radius = target.zr || target.rad
